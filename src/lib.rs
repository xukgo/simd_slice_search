...
impl MemchrSearcher {
    ...
    #[inline]
    pub fn inlined_search_in(&self, haystack: &[u8]) -> (Option<usize>,usize) {
        if haystack.is_empty() {
            return (Option::None,0);
        }

        (memchr(self.0, haystack),0)
    }

    /// Performs a substring search for the `needle` within `haystack`.
    pub fn search_in(&self, haystack: &[u8]) -> (Option<usize>,usize) {
...
trait Searcher<N: NeedleWithSize + ?Sized> {
    ...
    unsafe fn vector_search_in<V: Vector>(
        ...
        if remainder > 0 {
            let start = haystack.as_ptr().add(end - lane_unit);
            let mask = u32::MAX << (lane_unit - remainder);
...
mod tests {
    ...
    fn memchr_search(haystack: &[u8], needle: &[u8]) -> Option<usize> {
        MemchrSearcher::new(needle[0]).search_in(haystack).0
    }

    #[test]
    fn memchr_search_same() {
        assert!(memchr_search(b"f", b"f").is_some());
    }

    #[test]
    fn memchr_search_different() {
        assert!(!memchr_search(b"foo", b"b").is_some());
    }

    #[test]
    fn memchr_search_prefix() {
        assert!(memchr_search(b"foobar", b"f").is_some());
    }

    #[test]
    fn memchr_search_suffix() {
        assert!(memchr_search(b"foobar", b"r").is_some());
    }

    #[test]
    fn memchr_search_mutiple() {
        assert!(memchr_search(b"foobarfoo", b"o").is_some());
    }

    #[test]
    fn memchr_search_middle() {
        assert!(memchr_search(b"foobarfoo", b"b").is_some());
    }
