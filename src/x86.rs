...
impl<N: Needle> Avx2Searcher<N> {
    ...
    #[inline]
    #[target_feature(enable = "avx2")]
    unsafe fn sse2_2_search_in(&self, haystack: &[u8], end: usize) -> (Option<usize>,usize) {
        let hash = VectorHash::<__m16i>::from(&self.sse2_hash);
        (self.vector_search_in_avx2_version(haystack, end, &hash),__m16i::LANES)
    }

    #[inline]
    #[target_feature(enable = "avx2")]
    unsafe fn sse2_4_search_in(&self, haystack: &[u8], end: usize) -> (Option<usize>,usize) {
        let hash = VectorHash::<__m32i>::from(&self.sse2_hash);
        (self.vector_search_in_avx2_version(haystack, end, &hash),__m32i::LANES)
    }

    #[inline]
    #[target_feature(enable = "avx2")]
    unsafe fn sse2_8_search_in(&self, haystack: &[u8], end: usize) -> (Option<usize>,usize) {
        let hash = VectorHash::<__m64i>::from(&self.sse2_hash);
        (self.vector_search_in_avx2_version(haystack, end, &hash),__m64i::LANES)
    }

    #[inline]
    #[target_feature(enable = "avx2")]
    unsafe fn sse2_16_search_in(&self, haystack: &[u8], end: usize) -> (Option<usize>,usize) {
        (self.vector_search_in_avx2_version(haystack, end, &self.sse2_hash),__m128i::LANES)
    }

    #[inline]
    #[target_feature(enable = "avx2")]
    unsafe fn avx2_search_in(&self, haystack: &[u8], end: usize) -> (Option<usize>,usize) {
        (self.vector_search_in_avx2_version(haystack, end, &self.avx2_hash),__m256i::LANES)
    }

    /// Inlined version of `search_in` for hot call sites.
    #[inline]
    #[target_feature(enable = "avx2")]
    pub unsafe fn inlined_search_in(&self, haystack: &[u8]) -> (Option<usize>,usize) {
        if haystack.len() <= self.needle.size() {
            return if haystack == self.needle.as_bytes(){
                (Some(0),0)
            }else{
                (Option::None,0)
            }
    ...
    #[target_feature(enable = "avx2")]
    pub unsafe fn search_in(&self, haystack: &[u8]) -> (Option<usize>,usize) {
...
mod tests {
    ...
    impl crate::tests::TestSearcher for Avx2Searcher<&[u8]> {
        ...
        fn search_in(&self, haystack: &[u8]) -> bool {
           let tb =  unsafe { Avx2Searcher::search_in(self, haystack) };
           return tb.0.is_some();
        }
    }
    ...
    impl crate::tests::TestSearcher for DynamicAvx2Searcher<&[u8]> {
        ...
        fn search_in(&self, haystack: &[u8]) ->bool {
            let tb =  unsafe { DynamicAvx2Searcher::search_in(self, haystack) };
            return tb.0.is_some();
        }
    }
